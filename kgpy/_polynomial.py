        ...
        for d in range(self.degree + 1):
            for k in range(d + 1):
                for j in range(d + 1):
                    for i in range(d + 1):
                        if i + j + k == d:
                            name_x = ''.join(self.input_names[vectors.ix] * i) + ' '
                            name_y = ''.join(self.input_names[vectors.iy] * j) + ' '
                            name_z = ''.join(self.input_names[vectors.iz] * k) + ' '
    ...
    def from_lstsq_fit(
        ...
        return cls(
            x=Polynomial3D.from_lstsq_fit(
                ...
                degree=degree,
                input_names=input_names,
                output_name=output_names[vectors.ix],
            ),
            y=Polynomial3D.from_lstsq_fit(
                ...
                degree=degree,
                input_names=input_names,
                output_name=output_names[vectors.iy],
            ),
        )
