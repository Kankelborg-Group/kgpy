...
def percentile_filter(x: np.ndarray, percentile: float, kernel_size: tp.Tuple[int, ...]):
    ...
    return scipy.ndimage.filters.percentile_filter(x, percentile, size=kernel_size)

    # @numba.guvectorize([(numba.float32[:, :, :], numba.float32[:, :, :])], '(l, m, n) -> (l, m ,n)', target='cpu', nopython=True)
    # def gufunc(a, out):
    #     out[:, :, :] = scipy.ndimage.filters.percentile_filter(a, percentile, size=kernel_size)
    #
    # p = np.empty_like(x)
    # gufunc(x, p)
    #
